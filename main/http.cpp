...
static void SelectList(httpd_req_t *Req, const char *Name, const char **List,
		int Size, int Sel = 0) {
	char Line[64];
	int Len;
	...
	for (int Idx = 0; Idx < Size; Idx++) {
		...
		Len += Format_String(Line + Len, "\"");
		if (Idx == Sel)
			Len += Format_String(Line + Len, " selected>");
		else
			Len += Format_String(Line + Len, ">");
...
static void Page_Control_Row(httpd_req_t *Req, const char *Name,
		const int Index, const char *IndexChar) {
	Begin_Control_Row(Req, Name);

	httpd_resp_sendstr_chunk(Req,
			"<span class=\"page-checkbox\"><input type=\"checkbox\" onclick=\"pageCheckbox(this)\" class=\"page-checkbox-input\"");
...
static void ParmForm_Info(httpd_req_t *Req) {
	...
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"Info\">\n");

	Begin_Control_Row(Req, "Pilot");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Pilot\" size=\"10\" value=\"");
	if (Parameters.Pilot[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Pilot);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Crew");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Crew\" size=\"10\" value=\"");
	if (Parameters.Crew[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Crew);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Base airfield");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Base\" size=\"10\" value=\"");
	if (Parameters.Base[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Base);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Registration");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Reg\" size=\"10\" value=\"");
	if (Parameters.Reg[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Reg);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Manufacturer");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Manuf\" size=\"10\" value=\"");
	if (Parameters.Manuf[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Manuf);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Model");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Model\" size=\"10\" value=\"");
	if (Parameters.Model[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Model);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);
	...
	if (Parameters.Type[0])
		httpd_resp_sendstr_chunk(Req, Parameters.Type);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	httpd_resp_sendstr_chunk(Req,
			"<div class=\"submit-row\"><input type=\"submit\" value=\"Save\"></div>\n");
...
static void ParmForm_Acft(httpd_req_t *Req) {
	...
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"Acft\">\n");

	Begin_Control_Row(Req, "Address");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"Address\" size=\"10\" value=\"0x");
	Format_Hex(Line, (uint8_t)(Parameters.Address >> 16));
	Format_Hex(Line + 2, (uint16_t) Parameters.Address);
	...
	const char *AcftTypeTable[16] = { "Unknown", "(moto)Glider", "Tow-plane",
			"Helicopter", "Parachute", "Drop-plane", "Hang-glider",
			"Para-glider", "Powered-aircraft", "Jet-aircraft", "UFO", "Balloon",
			"Airship", "UAV/drone", "Ground support", "Static object" };
...
static void ParmForm_GPS(httpd_req_t *Req) // produce HTML form for GPS parameters
  ...
  httpd_resp_sendstr_chunk(Req, "<h2>GPS</h2>");
...
#endif
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"GPS\">\n");

	Begin_Control_Row(Req, "Nav. rate [Hz]");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"NavRate\" size=\"10\" value=\"");
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"NavMode\" size=\"10\" value=\"");
	...
	SelectList(Req, "manGeoidSepar", GeoidSeparTable, 2,
			Parameters.manGeoidSepar);

	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"GeoidSepar\" size=\"3\" value=\"");
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"GNSS\" size=\"10\" value=\"0x");
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"PPSdelay\" size=\"10\" value=\"");
...
static void ParmForm_Page(httpd_req_t *Req) // produce HTML form for parameters not included in other forms
	...
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"Pages\">\n");

	Begin_Control_Row(Req, "Altitude Unit");
	const char *AltitudeUnitTable[2] = { "meter", "feet" };
	SelectList(Req, "AltitudeUnit", AltitudeUnitTable, 2,
			Parameters.AltitudeUnit);
	End_Control_Row(Req);
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"hidden\" name=\"PageMask\" value=\"");
	Len = Format_UnsDec(Line, (uint8_t) Parameters.PageMask);
	httpd_resp_send_chunk(Req, Line, Len);
	httpd_resp_sendstr_chunk(Req, "\">");
	httpd_resp_sendstr_chunk(Req,
			"<span class=\"page-checkbox\">Show</span><span>Initial Page</span>");
	End_Control_Row(Req);
...
static void ParmForm_Other(httpd_req_t *Req) // produce HTML form for parameters not included in other forms
	...
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"Other\">\n");

	Begin_Control_Row(Req, "Freq. plan");
	const char *FreqPlanTable[6] = { "Auto", "Europe/Africa", "USA/Canada",
			"Australia/Chile", "New Zeeland", "Izrael" };
	SelectList(Req, "FreqPlan", FreqPlanTable, 6, Parameters.FreqPlan);
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Tx power [dBm]");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"TxPower\" size=\"10\" value=\"");
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"RFchipFreqCorr\" size=\"10\" value=\"");
	...
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"CONbaud\" size=\"10\" value=\"");
...
static void ParmForm_AP(httpd_req_t *Req) // Wi-Fi access point parameters { char Line[16]; int Len;
	...
	httpd_resp_sendstr_chunk(Req,
			"<form action=\"/parm.html\" method=\"POST\" id=\"AP\">\n");

	Begin_Control_Row(Req, "SSID");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"APname\" size=\"10\" value=\"");
	if (Parameters.APname[0])
		httpd_resp_sendstr_chunk(Req, Parameters.APname);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Password");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"APpass\" size=\"10\" value=\"");
	if (Parameters.APpass[0])
		httpd_resp_sendstr_chunk(Req, Parameters.APpass);
	httpd_resp_sendstr_chunk(Req, "\">");
	End_Control_Row(Req);

	Begin_Control_Row(Req, "Data port");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"APport\" size=\"10\" value=\"");
	...
	Begin_Control_Row(Req, "Tx power [dBm]");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"APtxPwr\" size=\"10\" value=\"");
	...
	Begin_Control_Row(Req, "Min. RSSI [dBm]");
	httpd_resp_sendstr_chunk(Req,
			"<input type=\"text\" name=\"APminSig\" size=\"10\" value=\"");
...
static void Table_System(httpd_req_t *Req) {
  ...
  Len+=Format_String(Line+Len, "MS5611");
	...
	Len = Format_String(Line,
			"<tr><td>Bluetooth serial port</td><td align=\"right\">");
#ifdef WITH_BT_SPP
  ...
  Len+=Format_String(Line+Len, "Yes");
	...
	Len = Format_String(Line,
			"<tr><td>Digital Buzzer</td><td align=\"right\">");
#ifdef WITH_BEEPER
  Len+=Format_String(Line+Len, "Yes");
#else
	Len += Format_String(Line + Len, "No");
  ...
  Len+=Format_String(Line+Len, "Yes");
	...
	Len = Format_String(Line,
			"<tr><td>Access Point (wifi)</td><td align=\"right\">");
#ifdef WITH_AP
	Len += Format_String(Line + Len, "Yes");
#else
  Len+=Format_String(Line+Len, "No");
...
static void Table_GPS(httpd_req_t *Req) {
	...
	Len += Format_UnsDec(Line + Len, GPS->Year + 2000, 4);
	Line[Len++] = '.';
	Len += Format_UnsDec(Line + Len, GPS->Month, 2);
	Line[Len++] = '.';
	...
	Len += Format_UnsDec(Line + Len, GPS->Hour, 2);
	Line[Len++] = ':';
	Len += Format_UnsDec(Line + Len, GPS->Min, 2);
	Line[Len++] = ':';
	Len += Format_UnsDec(Line + Len, GPS->Sec, 2);
	Line[Len++] = '.';
	...
	Len += Format_UnsDec(Line + Len, ((uint16_t) 10 * GPS_SatSNR + 2) / 4, 2,
			1);
	...
	if (GPS->hasBaro) {
		Len = Format_String(Line,
				"<tr><td>Pressure Alt.</td><td align=\"right\">");
		...
		Len = Format_String(Line,
				"<tr><td>Temperature</td><td align=\"right\">");
...
static void Table_LookOut(httpd_req_t *Req) {
	...
	httpd_resp_sendstr_chunk(Req,
			"<table class=\"table table-striped table-bordered\">\n");
	httpd_resp_sendstr_chunk(Req,
			"<thead><tr><th>LookOut</th><th>Time Margin</th><th>Distance</th></tr></thead>\n<tbody>\n");
...
static void Table_Relay(httpd_req_t *Req) {
	...
	httpd_resp_sendstr_chunk(Req,
			"<table class=\"table table-striped table-bordered\">\n");
	httpd_resp_sendstr_chunk(Req,
			"<thead><tr><th>Relay</th><th>Rank</th><th>[sec]</th></tr></thead>\n<tbody>\n");
...
static void Table_RF(httpd_req_t *Req) {
  ...
  Len+=Format_String(Line+Len, "sx1262");
	...
	Len += Format_SignDec(Line + Len, (int32_t) Parameters.RFchipFreqCorr, 2,
			1);
	...
	Len += Format_UnsDec(Line + Len,
			(uint16_t)(RF_FreqPlan.getCenterFreq() / 100000), 3, 1);
...
static void Html_Start(httpd_req_t *Req, const char *Title,
		const uint8_t ActiveMenuIndex) {
	httpd_resp_sendstr_chunk(Req,
			"\
<!DOCTYPE html>\n\
<html>\n\
...
");
	...
	if (ActiveMenuIndex == 1)
		httpd_resp_sendstr_chunk(Req, " class=\"active\"");
...
static int LogFileName(char *Name, time_t Time, const char *Ext = 0) {
	...
	if (Ext)
		Len += Format_String(Name + Len, Ext);
	Name[Len] = 0;
	return Len;
}

// send give log file in the IGC format
static esp_err_t SendLog_IGC(httpd_req_t *Req, const char *FileName,
		uint32_t FileTime) {
	char ContDisp[64];
	...
	Len += LogFileName(ContDisp + Len, FileTime, ".IGC");
	ContDisp[Len++] = '\"';
	ContDisp[Len] = 0;
...
#endif
	Len +=
			Format_String(Line + Len,
					"HFRFWFIRMWAREVERSION:ESP32-OGN-TRACKER " __DATE__ " " __TIME__ "\n");
#ifdef WITH_BMP180
  ...
  Len+=Format_String(Line+Len, "HFPRSPRESSALTSENSOR:BME280/BMP280\n"); // pressure sensor
	...
	Len = 0;
	for (;;) {
		if (fread(&Packet, Packet.Bytes, 1, File) != 1)
			break;          // read the next packet
		if (!Packet.isCorrect())
			continue;
		...
		bool Own = Packet.Packet.Header.Address == Parameters.Address
				&& Packet.Packet.Header.AddrType == Parameters.AddrType; //
		if (Own && !Packet.Packet.Header.NonPos
				&& !Packet.Packet.Header.Encrypted)
	...
	Len = 0;
	Line[Len++] = 'G';                     // G-record, not signed yet, just MD5
...
static esp_err_t SendLog_APRS(httpd_req_t *Req, const char *FileName,
		uint32_t FileTime) {
	char ContDisp[64];
	char Line[1000];
	int Len = 0;
	Len = Format_String(ContDisp, "attachement; filename=\"");
	Len += LogFileName(ContDisp + Len, FileTime, ".aprs");
	ContDisp[Len++] = '\"';
	ContDisp[Len] = 0;
	httpd_resp_set_hdr(Req, "Content-Disposition", ContDisp);
	httpd_resp_set_type(Req, "text/plain");
...
static esp_err_t SendLog_TLG(httpd_req_t *Req, const char *FileName,
		uint32_t FileTime) {
	char ContDisp[64];
	char Line[512];
	int Len;
	Len = Format_String(ContDisp, "attachement; filename=\"");
	Len += Parameters.getAprsCall(ContDisp + Len);
	ContDisp[Len++] = '_';
	Len += Format_Hex(ContDisp + Len, FileTime);
	Len += Format_String(ContDisp + Len, ".TLG");
	ContDisp[Len++] = '\"';
	ContDisp[Len] = 0;
	httpd_resp_set_hdr(Req, "Content-Disposition", ContDisp);
...
static esp_err_t log_get_handler(httpd_req_t *Req) {
	...
	if (URLlen) {
		...
		char Format[8] = { 0 };
		httpd_query_key_value(URL, "Format", Format, 8);
		free(URL);
		if (SendFile) {
			AddPath(FullName, Name, Path);
			uint32_t Time = FlashLog_ReadShortFileTime(Name);
			if (Time) {
				if (strcmp(Format, "APRS") == 0)
					return SendLog_APRS(Req, FullName, Time);
				if (strcmp(Format, "IGC") == 0)
					return SendLog_IGC(Req, FullName, Time);
	...
	if (Dir == 0) {
		httpd_resp_sendstr_chunk(Req,
				"<p>Cannot open the log directory !</p>\n");
		httpd_resp_send_chunk(Req, 0, 0);
		return ESP_OK;
	}
	for (;;) {
	...
	for (size_t Idx = 0; Idx < FileList.size(); Idx++) {
		...
		Len += Format_String(Line + Len, Name);
		Len += Format_String(Line + Len,
				"</a></td><td><a href=\"/log.html?Format=APRS&File=");
		Len += Format_String(Line + Len, Name);
		Len += Format_String(Line + Len, "\">APRS</a></td>");
		Len += Format_String(Line + Len,
				"<td><a href=\"/log.html?Format=IGC&File=");
		Len += Format_String(Line + Len, Name);
		Len += Format_String(Line + Len,
				"\">IGC</a></td><td align=\"center\">");
...
static const httpd_uri_t HTTPtop = { .uri = "/", .method = HTTP_GET, .handler =
		top_get_handler, .user_ctx = 0 };

static const httpd_uri_t HTTPlogo = { .uri = "/logo.jpeg", .method = HTTP_GET,
		.handler = logo_get_handler, .user_ctx = 0 };

static const httpd_uri_t HTTPparm = { .uri = "/parm.html", .method = HTTP_GET,
		.handler = parm_get_handler, .user_ctx = 0 };

static const httpd_uri_t HTTPparmPost = { .uri = "/parm.html", .method =
		HTTP_POST, .handler = parm_post_handler, .user_ctx = 0 };

static const httpd_uri_t HTTPlog = { .uri = "/log.html", .method = HTTP_GET,
		.handler = log_get_handler, .user_ctx = 0 };
