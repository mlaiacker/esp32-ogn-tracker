...
{
  if (!TRX.DIO0_isOn())
    return 0; // DIO0 line HIGH signals a new packet has arrived
#ifdef WITH_LED_RX
  LED_RX_Flash(20);
#endif
  ...
  TRX.ClearIrqFlags();
  return 1;
} // return: 1 packet we have received
...
{
  if (PacketByte == 0)
    return 0; // if no packet to send: simply return
...
static void TimeSlot(uint8_t TxChan, uint32_t SlotLen, const uint8_t *PacketByte, uint8_t Rx_RSSI, uint8_t MaxWait = 8, uint32_t TxTime = 0)
{
  ...
  if ((TxTime == 0) || (TxTime >= MaxTxTime))
    TxTime = RX_Random % MaxTxTime;                                   // if TxTime out of limits, setup a random TxTime
  ...
  if ((TX_Credit > 0) && Parameters.TxPower != (-32) && (PacketByte)) // when packet to transmit is given and there is still TX credit left:
    if (Transmit(TxChan, PacketByte, Rx_RSSI, MaxWait))
      TX_Credit -= 5; // attempt to transmit the packet
...
#endif
  return Version;
} // read the RF chip version and return it
...
extern "C" void vTaskRF(void *pvParameters)
{
  ...
  for (;;)
  {
    ...
    {
      if (WAN_RespLeft <= 5)
        WANdev.State--; // if time below 5 ticks we have not enough time
      else if (WAN_RespLeft < 200)
      {
        WANrx = 1;
      } // if more than 200ms then we can't wait this long now
    ...
    if (WANrx) // if reception expected from WAN
    {
      ...
      TRX.LoRa_InvertIQ(1);
      TRX.LoRa_setCRC(0);
      TRX.LoRa_setIRQ(0);            // setup for WAN RX
      ...
      if (Wait) // if no timeout thus a packet has been received.
      {
        ...
        xSemaphoreGive(CONS_Mutex);
        if (WANdev.State == 1)
          WANdev.procJoinAccept(WAN_RxPacket); // if join-request state then expect a join-accept packet
        else if (WANdev.State == 3)
          RxLen = WANdev.procRxData(WAN_RxPacket); // if data send then respect ACK and/or downlink data packet
        WANdev.RxSilent = 0;
      }
      else // if no packet received then retreat the State
      {
        WANdev.State--;
        WANdev.RxSilent++;
        if (WANdev.RxSilent > 30)
          WANdev.Disconnect();
      }                               // count silence when reception expected, if too many then disconnect
    ...
    {
      uint32_t RxRssiSum = 0;
      uint16_t RxRssiCount = 0; // measure the average RSSI for lower frequency
      do
      {
    ...
    uint32_t RxRssiSum = 0;
    uint16_t RxRssiCount = 0; // measure the average RSSI for the upper frequency
    do
    {
    ...
    TX_Credit += 10;
    if (TX_Credit > 3600000)
      TX_Credit = 600000; // [ms] count the transmission credit

    XorShift32(RX_Random);
    uint32_t TxTime = (RX_Random & 0x3F) + 1;
    TxTime *= 6;
    ...
    if (TxPkt0)
      TxPktData0 = TxPkt0->Byte(); // if 1st is not NULL then get its data
    if (TxPkt1)
      TxPktData1 = TxPkt1->Byte(); // if 2nd if not NULL then get its data
    else
      TxPktData1 = TxPktData0; // but if NULL then take copy of the 1st packet

    if (TxPkt0) // if 1st packet is not NULL
    {
      if ((RX_Channel != TxChan) && (TxPkt0->Packet.Header.Relay == 0))
      {
        const uint8_t *Tmp = TxPktData0;
        TxPktData0 = TxPktData1;
        TxPktData1 = Tmp;
      } // swap 1st and 2nd packet data
    }
    ...
    if (FNTpkt)                                         // was there any ?
    {
      ...
      for (uint8_t Wait = 50; Wait; Wait--) //
      {
        ...
        if ((Stat & 0x0B) == 0)
          break;
      } // 0:signal-det, 1:signal-sync, 3:header-valid
    ...
    XorShift32(RX_Random);
    TxTime = (RX_Random & 0x3F) + 1;
    TxTime *= 6; // [ms] (1..64)*6 = 6..384ms
#ifdef WITH_LORAWAN
    bool WANtx = 0;
    ...
    if (WANtx)
    {
      ...
      TRX.setChannel(WANdev.Chan); // set the channel
      TRX.LoRa_InvertIQ(0);
      TRX.LoRa_setCRC(1);                   // setup for WAN TX
      ...
      {
        uint8_t *TxPacket;
        TxPktLen = WANdev.getJoinRequest(&TxPacket); // produce Join-Request packet
        TRX.LoRa_SendPacket(TxPacket, TxPktLen);
        RespDelay = 5000; // transmit join-request packet
        WAN_BackOff = 48 + (RX_Random % 21);
        XorShift32(RX_Random);
      }
      else if (WANdev.State == 2)
      {
        ...
        {
          OGN1_Packet *OGN = (OGN1_Packet *)PktData;
          if (!OGN->Header.Encrypted)
            OGN->Dewhiten();
          uint8_t *TxPacket;
          bool Short = !OGN->Header.NonPos && !OGN->Header.Encrypted && OGN->Header.AddrType == 3 && OGN->Header.Address == (uint32_t)(getUniqueAddress() & 0x00FFFFFF);
          if (Short)
          {
