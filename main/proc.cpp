...
void Sound_TrafficWarn(const LookOut_Target *Tgt)
{
  ...
  // CONS_UART_Write(' ');
  uint16_t DirIdx = (Bearing + 0x800) >> 12;
  DirIdx &= 0x0F;
  Format_String(CONS_UART_Write, Dir[DirIdx]);
  CONS_UART_Write(' ');
  uint16_t RelDirIdx = (RelBearing + 0x1000) >> 13;
  RelDirIdx &= 0x07;
...
{
  OGN_LogPacket<OGN_Packet> *LogPacket = FlashLog_FIFO.getWrite();
  if (LogPacket == 0)
    return -1;                        // allocate new packet in the LOG_FIFO
...
{
  if (RelayQueue.Sum == 0)
    return 0;                                  // if no packets in the relay queue
  ...
  if (RelayQueue.Packet[Idx].Rank == 0)
  ...
  if (!Packet->Packet.Header.Encrypted)
    Packet->Packet.Whiten(); // whiten but only for non-encrypted packets
...
} // remove packets 20(default) seconds into the past
...
static int ReadInfo(OGN1_Packet &Packet)
{
  ...
  for (ParmIdx = InfoParmIdx;;)
  {
    ...
    { // printf("Parm[%d]=%s\n", ParmIdx, Parm);
      if (Parm[0])
      {
        int Add = Packet.addInfo(Parm, ParmIdx);
        if (Add == 0)
          break;
      }
    }
    ParmIdx++;
    if (ParmIdx >= Parameters.InfoParmNum)
      ParmIdx = 0;
    if (ParmIdx == InfoParmIdx)
      break;
  }
  InfoParmIdx = ParmIdx;
  Packet.setInfoCheck();
  return Packet.Info.DataChars;
} // zero => no info parameters were stored
...
static void ReadStatus(OGN_Packet &Packet)
{
  ...
  */
  ...
  if (MCU_Vbatt < 3600)
  {
    uint16_t FlashLen = 3600 - MCU_Vbatt;
    if (FlashLen > 250)
      FlashLen = 250;
...
#ifdef WITH_SX1262
  if (Packet.Status.Pressure == 0)
    Packet.clrTemperature();
#else
  if (Packet.Status.Pressure == 0)
    Packet.EncodeTemperature(TRX.chipTemp * 10); // [0.1degC]
#endif
...
static void ProcessRxPacket(OGN_RxPacket<OGN_Packet> *RxPacket, uint8_t RxPacketIdx, uint32_t RxTime) // process every (correctly) received packet
{
  ...
  uint8_t MyOwnPacket = (RxPacket->Packet.Header.Address == Parameters.Address) && (RxPacket->Packet.Header.AddrType == Parameters.AddrType);
  if (MyOwnPacket)
    return;                                                      // don't process my own (relayed) packets
...
//     xSemaphoreGive(CONS_Mutex);
    ...
    if (Tgt)
      Warn = Tgt->WarnLevel; // remember warning level of this target
...
#ifdef WITH_BEEPER
    if (KNOB_Tick > 12)
      Play(Play_Vol_1 | Play_Oct_2 | (7 + 2 * Warn), 3 + 16 * Warn);
#endif
#else // if not WITH_LOOKOUT
#ifdef WITH_BEEPER
    if (KNOB_Tick > 12)
      Play(Play_Vol_1 | Play_Oct_2 | 7, 3);                                         // if Knob>12 => make a beep for every received packet
#endif
#endif // WITH_LOOKOUT
    bool Signif = PrevRxPacket == 0;
    if (!Signif)
      Signif = OGN_isSignif(&(RxPacket->Packet), &(PrevRxPacket->Packet)); // compare against previous packet of same ID from the relay queue
#ifdef WITH_APRS
    if (Signif)
      APRSrx_FIFO.Write(*RxPacket); // APRS queue for received packets
#endif
#ifdef WITH_LOG
    if (Signif)
      FlashLog(RxPacket, RxTime); // log only significant packets
#endif
#ifdef WITH_SDLOG
    if (Signif || Warn)
      IGClog_FIFO.Write(*RxPacket);
    ...
    vTaskPROC(void *pvParameters)
{
  ...
  for (;;)
  {
    ...
    if (msTime < 340)
      SlotTime--; // lasts up to 0.300sec after the PPS

    if (SlotTime == PrevSlotTime)
      continue;              // stil same time slot, go back to RX processing
    ...
    if (Parameters.Reg[0])
      GDL_REPORT.setAcftCall(Parameters.Reg);
    // else GDL_REPORT.setAcftCall();
    if (Position && Position->isValid())
      Position->Encode(GDL_REPORT);
    xSemaphoreTake(CONS_Mutex, portMAX_DELAY);
    ...
    if (Position)
    {
      ...
      {
        StatPacket.Packet.Status.FixQuality = 0;
        StatPacket.Packet.Status.Satellites = 0;
      } // or lack of the GPS lock
      ...
      if (Position && Position->isReady && (!Position->Sent) && Position->isValid())
      {
        ...
        if (Parameters.FreqPlan == 0)
          ...
          RF_FreqPlan.setPlan(Parameters.FreqPlan);
#ifdef DEBUG_PRINT
        xSemaphoreTake(CONS_Mutex, portMAX_DELAY);
        ...
        {
          PosPacket.Packet.Header.Encrypted = 1;
        }                                  // then set the flg in the header
#endif                                     // WITH_ENCRYPT
        ...
        {
          while (BestResid >= 500)
            BestResid -= 1000; // remove full seconds
        ...
        if (Parameters.Encrypt)
          TxPacket->Packet.Encrypt(Parameters.EncryptKey); // if encryption is requested then encrypt
        else
          TxPacket->Packet.Whiten(); // otherwise only whiten
#else                                // WITH_ENCRYPT
        ...
        else
        {
          ...
          TxBackOff = 0;
          if (AverSpeed < 10 && Parameters.AcftType != 3 && Parameters.AcftType != 0xD)
            TxBackOff += 3 + (RX_Random & 0x1);
          if (TX_Credit <= 0)
            TxBackOff += 1;
        }
        Position->Sent = 1;
        ...
        uint8_t Warn = 0;
        if (Tgt)
          Warn = Tgt->WarnLevel;                 // what is the warning level ?
      ...
      {
        if ((SlotTime - PosTime) >= 30)
        {
          PosPacket.Packet.Position.Time = 0x3F;
        } // if no valid position for more than 30 seconds then set the time as unknown for the transmitted packet
        OGN_TxPacket<OGN_Packet> *TxPacket = RF_TxFIFO.getWrite();
...
#ifdef DEBUG_PRINT
      // char Line[128];
      Line[0] = '0' + RF_TxFIFO.Full();
      Line[1] = ' ';              // print number of packets in the TxFIFO
      ...
      if (StatTxBackOff == 0 && RF_TxFIFO.Full() < 2) // decide whether to transmit the status/info packet
      {
        ...
        {
          doTx = ReadInfo(StatPacket.Packet);
        } // and overwrite the StatPacket with the Info data
        if (doTx)
        {
