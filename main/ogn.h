...
static bool OGN_isSignif(const OGNx_Packet *Packet, const OGNy_Packet *PrevPacket) // is significant: decide whether to store it or not
{
  ...
  if (abs(DistDeltaH) >= 200)
    return 1;                                                      // if extrapolation error more than 50m
  ...
  if (abs(CFaccel) >= 25)
    return 1;                                                                           // CFaccel at or above 5m/s^2 (0.5g)
...
public:
  ...
  {
    uint8_t Len = 0;
    Out[Len++] = HexDigit(Packet.Position.AcftType);
    Out[Len++] = ':';
    Out[Len++] = '0' + Packet.Header.AddrType;
    Out[Len++] = ':';
    ...
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, (uint32_t)Packet.DecodeAltitude());
    Out[Len++] = 'm';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, Packet.DecodeSpeed(), 2, 1);
    Out[Len++] = 'm';
    Out[Len++] = '/';
    Out[Len++] = 's';
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, Packet.DecodeClimbRate(), 2, 1);
    Out[Len++] = 'm';
    Out[Len++] = '/';
    Out[Len++] = 's';
    Out[Len++] = '\n';
...
public:
  ...
  {
    uint8_t ByteIdx = 0;
    const uint32_t *WordPtr = Packet.Word();
    for (uint8_t WordIdx = 0; WordIdx < Words; WordIdx++)
    {
      uint32_t Word = WordPtr[WordIdx];
      for (int Idx = 0; Idx < 4; Idx++)
      {
        if (ByteIdx >= Bytes)
          break;
        ErrPatt[ByteIdx] = OtherPacket[ByteIdx] ^ Word;
        ByteIdx++;
        Word >>= 8;
      }
    }
  ...
  void calcRelayRank(int32_t RxAltitude) // [m] altitude of reception
  {
    ...
    if (Packet.Header.NonPos)
      return; // only relay position packets
    if (Packet.Position.Time >= 60)
      return; // don't relay packets with unknown time - but maybe we should ?
    if (Packet.Header.Relay)
      return;                      // no rank for relayed packets (only single relay)
    if (RxRSSI > 128)              // [-0.5dB] weaker signal => higher rank
      Rank += (RxRSSI - 128) >> 2; // 1point/2dB less signal
    if (Packet.Header.Encrypted)
      return;                              // for exncrypted packets we only take signal strength
  ...
  uint8_t ReadPOGNT(const char *NMEA)
  {
    ...
    if ((AddrType < 0) || (AddrType >= 4))
      return -1;
    Packet.Header.AddrType = AddrType;
    Len += 2;

    uint32_t Addr;
    int8_t Ret = Read_Hex(Addr, NMEA + Len);
    if (Ret <= 0)
      return -1;
    if (NMEA[Len + Ret] != ',')
      return -1;
    ...
    if (NMEA[Len + 10] != ',')
      return -1;
    int8_t Deg = Read_Dec2(NMEA + Len);
    if (Deg < 0)
      return -1;
    int8_t Min = Read_Dec2(NMEA + Len + 2);
    if (Min < 0)
      return -1;
    if (NMEA[Len + 4] != '.')
      return -1;
    int16_t Frac = Read_Dec4(NMEA + Len + 5);
    if (Frac < 0)
      return -1;
    ...
    if (NMEA[Len + 11] != ',')
      return -1;
    Deg = Read_Dec3(NMEA + Len);
    if (Deg < 0)
      return -1;
    Min = Read_Dec2(NMEA + Len + 3);
    if (Min < 0)
      return -1;
    if (NMEA[Len + 5] != '.')
      return -1;
    Frac = Read_Dec4(NMEA + Len + 6);
    if (Frac < 0)
      return -1;
    Sign = NMEA[Len + 10];
    ...
    int32_t AltDiff = 0;
    Ret = Read_SignDec(AltDiff, NMEA + Len);
    if (Ret < 0)
      return -1;
    ...
    int32_t Climb = 0;
    Ret = Read_Float1(Climb, NMEA + Len);
    if (Ret < 0)
      return -1;
    ...
    int32_t Speed = 0;
    Ret = Read_Float1(Speed, NMEA + Len);
    if (Ret < 0)
      return -1;
    ...
    int32_t Heading = 0;
    Ret = Read_Float1(Heading, NMEA + Len);
    if (Ret < 0)
      return -1;
    ...
    int32_t TurnRate = 0;
    Ret = Read_Float1(TurnRate, NMEA + Len);
    if (Ret < 0)
      return -1;
    ...
    int32_t RSSI = 0;
    Ret = Read_SignDec(RSSI, NMEA + Len);
    if (Ret < 0)
      return -1;
    RxRSSI = (-2 * RSSI);
    if (NMEA[Len + Ret] != ',')
  ...
  uint8_t WritePFLAA(char *NMEA, uint8_t Status, int32_t RefLat, int32_t RefLon, int32_t RefAlt, uint16_t LatCos)
  {
    ...
    if (Packet.calcDistanceVector(LatDist, LonDist, RefLat, RefLon, LatCos) < 0)
      return 0; // return zero, when distance too large
    int32_t AltDist = Packet.DecodeAltitude() - RefAlt;
    return WritePFLAA(NMEA, Status, LatDist, LonDist, AltDist, Status);
...
#ifdef WITH_SKYDEMON
    if (AddrType != 1)
      AddrType = 2; // SkyDemon only accepts 1 or 2
#endif
...
public:
  ...
  uint8_t getSymbol(uint16_t Idx)
  {
    ...
    {
      uint8_t WordIdx = Idx >> 5;
      uint8_t BitIdx = Idx & 31;
      uint32_t Mask = 1;
      Mask <<= BitIdx;
      if (Word[WordIdx] & Mask)
        Symbol |= SymbMask;
      SymbMask <<= 1;
    }
    return Gray(Symbol);
...
public:
  ...
  void reCalc(void) // find the lowest rank and calc. the sum of all ranks
  {
    ...
    LowIdx = 0;                              // take minimum at the first slot
  ...
  void decrRank(uint8_t Idx, uint8_t Decr = 1) // decrement rank of given slot
  {
    uint8_t Rank = Packet[Idx].Rank;
    if (Rank == 0)
      return; // if zero already: do nothing
    if (Decr > Rank)
      Decr = Rank; // if to decrement by more than the rank already: reduce the decrement
    Rank -= Decr;
    Sum -= Decr; // decrement the rank and the sum of ranks
    if (Rank < Low)
    {
      Low = Rank;
      LowIdx = Idx;
    } // if new minimum: update the minimum.
    Packet[Idx].Rank = Rank;
  } // update the rank of this slot
  ...
  uint8_t Print(char *Out)
  {
    ...
    for (uint8_t Idx = 0; Idx < Size; Idx++) // loop through the slots
    {
      uint8_t Rank = Packet[Idx].Rank;
      Out[Len++] = ' ';
      Len += Format_Hex(Out + Len, Rank); // print the slot Rank
      if (Rank)                           // if Rank is none-zero
      {
        Out[Len++] = '/';
        Len += Format_Hex(Out + Len, Packet[Idx].Packet.getAddressAndType()); // print address-type and address
        Out[Len++] = ':';
        if (Packet[Idx].Header.Encrypted)
          Len += Format_String(Out + Len, "ee");
        else
          Len += Format_UnsDec(Out + Len, Packet[Idx].Packet.Position.Time, 2); // [sec] print time
      }
    }
    Out[Len++] = ' ';
    Len += Format_Hex(Out + Len, Sum); // sum of all Ranks
    Out[Len++] = '/';
    Len += Format_Hex(Out + Len, LowIdx); // index of the lowest Rank or a free slot
    Out[Len++] = '\n';
    Out[Len] = 0;
    return Len;
  }
...
public:
  ...
  int8_t decrTime(void) // decrement HH:MM:SS by one second
  {
    ...
    Hour = 24;
    return -1;
  } // return 1 if date needs to be decremented

  int32_t calcTimeDiff(const GPS_Time &RefTime) const
  {
    int32_t TimeDiff = msDayTime() - RefTime.msDayTime(); // [ms]
    if (TimeDiff < (-(int32_t)mSecsPerDay / 2))
      TimeDiff += mSecsPerDay; // wrap-around one day
    else if (TimeDiff >= (int32_t)mSecsPerDay / 2)
      TimeDiff -= mSecsPerDay;
    return TimeDiff;
  } // [ms]
  ...
  void incrTimeDate(void)
  {
    if (incrTime())
      incrDate();
  }
  void decrTimeDate(void)
  {
    if (decrTime())
      decrDate();
  }
  ...
  void copyTimeDate(GPS_Time &RefTime)
  {
    copyTime(RefTime);
    copyDate(RefTime);
  }
  ...
  {
    return Times60((uint32_t)(Times60((uint16_t)Hour) + Min)) + Sec;
  } // this appears to save about 100 bytes of code on STM32
  ...
  void setUnixTime(uint32_t Time) // works except for the 1.1.2000
  {
    ...
    Hour = DayTime / SecsPerHour;
    DayTime -= (uint32_t)Hour * SecsPerHour; //
    Min = DayTime / SecsPerMin;
    DayTime -= (uint16_t)Min * SecsPerMin;
    Sec = DayTime;
    mSec = 0;
    ...
    Month = Days / 31;
    Day = Days - (uint16_t)Month * 31 + 1;
    Month++;
    uint32_t CheckTime = getUnixTime();
    if (CheckTime < Time)
      incrDate((Time - CheckTime) / SecsPerDay);
  }
  ...
  int8_t ReadTime(const char *Value) // read the Time field: HHMMSS.sss and check if it is a new one or the same one
  {
    ...
    if (Hour < 0)
      return -1; // read hour (two digits), return when invalid
    if (Prev != Hour)
      Same = 0;
    Value += 2;
    if (Value[0] == ':')
    ...
    Min = Read_Dec2(Value);
    if (Min < 0)
      return -1; // read minute (two digits), return when invalid
    if (Prev != Min)
      Same = 0;
    Value += 2;
    if (Value[0] == ':')
    ...
    Sec = Read_Dec2(Value);
    if (Sec < 0)
      return -1; // read second (two digits), return when invalid
    Value += 2;
    if (Prev != Sec)
      Same = 0;
    int16_t mPrev = mSec;
    ...
    {
      uint16_t Frac = 0;
      int8_t Len = Read_UnsDec(Frac, Value + 1);
      if (Len < 1)
        return -1; // read the fraction, return when invalid
      if (Len == 1)
        mSec = Frac * 100;
      else if (Len == 2)
        mSec = Frac * 10;
    ...
    if (mPrev != mSec)
      Same = 0; // return 0 when time is valid but did not change
    return Same;
  ...
  int8_t ReadDate(const char *Param) // read the field DDMMYY
  {
    Day = Read_Dec2(Param);
    if (Day < 0)
      return -1; // read calendar year (two digits - thus need to be extended to four)
    Param += 2;
    if (Param[0] == '/')
      Param++;
    Month = Read_Dec2(Param);
    if (Month < 0)
      return -1; // read calendar month
    Param += 2;
    if (Param[0] == '/')
      Param++;
    Year = Read_Dec2(Param);
    if (Year < 0)
      return -1; // read calendar day
    return 0;
  } // return 0 when field valid and was read correctly
...
public:
  ...
  {
    if (!isTimeValid())
      return 0; // is GPS time valid/present ?
    if (!isDateValid())
      return 0; // is GPS date valid/present ?
    if (FixQuality == 0)
      return 0; // Fix quality must be 1=GPS or 2=DGPS
    if (FixMode == 1)
      return 0; // if GSA says "no lock" (when GSA is not there, FixMode=0)
    if (Satellites <= 0)
      return 0; // if number of satellites none or invalid
    return 1;
  }
  ...
  void copyBaro(GPS_Position &RefPos, int16_t dTime = 0)
  {
    ...
    if (dTime)
    {
      ...
      if (Pressure)
        Pressure += 4000 * dAlt / Atmosphere::PressureLapseRate(Pressure / 4, Temperature);
    } // [0.25Pa] ([Pa], [0.1degC])
    hasBaro = 1;
  }

#ifndef __AVR__ // there is not printf() with AVR
  void PrintDateTime(void) const
  {
    printf("%02d.%02d.%04d %02d:%02d:%06.3f", Day, Month, 2000 + Year, Hour, Min, Sec + 0.001 * mSec);
  }
  ...
  int Print(char *Out) const
  {
    ...
    printf(" "); // Len+=sprintf(Out+Len, " = %10ld.%02dsec\n", (long int)UnixTime, FracSec);
  ...
  int PrintLine(char *Out) const
  {
    ...
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, (uint16_t)Hour, 2);
    Out[Len++] = ':';
    Len += Format_UnsDec(Out + Len, (uint16_t)Min, 2);
    Out[Len++] = ':';
    Len += Format_UnsDec(Out + Len, (uint16_t)Sec, 2);
    Out[Len++] = '.';
    Len += Format_UnsDec(Out + Len, (uint16_t)mSec, 3);
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, (uint16_t)FixQuality);
    Out[Len++] = '/';
    Len += Format_UnsDec(Out + Len, (uint16_t)FixMode);
    Out[Len++] = '/';
    Len += Format_UnsDec(Out + Len, (uint16_t)Satellites, 2);
    Out[Len++] = ' ';
    ...
    Out[Len++] = ' ';
    Out[Len++] = '[';
    Len += Format_SignDec(Out + Len, Latitude / 6, 7, 5);
    Out[Len++] = ',';
    Len += Format_SignDec(Out + Len, Longitude / 6, 8, 5);
    Out[Len++] = ']';
    Out[Len++] = 'd';
    Out[Len++] = 'e';
    Out[Len++] = 'g';
    ...
    Out[Len++] = 'm';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, Speed, 2, 1);
    Out[Len++] = 'm';
    Out[Len++] = '/';
    Out[Len++] = 's';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, Heading, 4, 1);
    Out[Len++] = 'd';
    Out[Len++] = 'e';
    Out[Len++] = 'g';
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, ClimbRate, 2, 1);
    Out[Len++] = 'm';
    Out[Len++] = '/';
    Out[Len++] = 's';
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, TurnRate, 2, 1);
    Out[Len++] = 'd';
    Out[Len++] = 'e';
    Out[Len++] = 'g';
    Out[Len++] = '/';
    ...
    {
      Out[Len++] = ' ';
      Len += Format_SignDec(Out + Len, Temperature, 2, 1);
      Out[Len++] = 'C';
      Out[Len++] = ' ';
      Len += Format_UnsDec(Out + Len, Pressure / 4);
      Out[Len++] = 'P';
      Out[Len++] = 'a';
      Out[Len++] = ' ';
      Len += Format_SignDec(Out + Len, StdAltitude, 2, 1);
      Out[Len++] = 'm';
    }
    Out[Len++] = '\n';
    Out[Len++] = 0;
  ...
  int8_t ReadUBX(UBX_RxMsg &RxMsg)
  {
    ...
    if (RxMsg.isNAV_TIMEUTC())
      return ReadUBX_NAV_TIMEUTC(RxMsg);
    if (RxMsg.isNAV_POSLLH())
      return ReadUBX_NAV_POSLLH(RxMsg);
    if (RxMsg.isNAV_SOL())
      return ReadUBX_NAV_SOL(RxMsg);
    return 0;
  }
  ...
  { // if(RxMsg.isGPGGA()) return ReadGGA(RxMsg);
    ...
    if (RxMsg.isPGRMZ())
      return ReadPGRMZ(RxMsg); // (pressure) altitude
  ...
  {
    if (RxMsg.Parms < 14)
      return -2;                                           // no less than 14 paramaters
    ...
    if (FixQuality < 0)
      FixQuality = 0;                                       // fix quality: 0=invalid, 1=GPS, 2=DGPS
    Satellites = Read_Dec2((const char *)RxMsg.ParmPtr(6)); // number of satellites
    if (Satellites < 0)
      Satellites = Read_Dec1(RxMsg.ParmPtr(6)[0]);
    if (Satellites < 0)
      Satellites = 0;
  ...
  {
    if ((memcmp(GGA, "$GPGGA", 6) != 0) && (memcmp(GGA, "$GNGGA", 6) != 0))
      return -1; // check if the right sequence
    uint8_t Index[20];
    if (IndexNMEA(Index, GGA) < 14)
      return -2; // index parameters and check the sum
    hasGPS = ReadTime(GGA + Index[0]) > 0;
    FixQuality = Read_Dec1(GGA[Index[5]]);
    if (FixQuality < 0)
      FixQuality = 0;                       // fix quality
    Satellites = Read_Dec2(GGA + Index[6]); // number of satellites
    if (Satellites < 0)
      Satellites = Read_Dec1(GGA[Index[6]]);
    if (Satellites < 0)
      Satellites = 0;
  ...
  {
    if ((memcmp(GSA, "$GPGSA", 6) != 0) && (memcmp(GSA, "$GNGSA", 6) != 0))
      return -1; // check if the right sequence
    uint8_t Index[20];
    if (IndexNMEA(Index, GSA) < 17)
      return -2; // index parameters and check the sum
  ...
  */
  ...
  {
    if ((memcmp(RMC, "$GPRMC", 6) != 0) && (memcmp(RMC, "$GNRMC", 6) != 0))
      return -1; // check if the right sequence
    uint8_t Index[20];
    if (IndexNMEA(Index, RMC) < 11)
      return -2; // index parameters and check the sum
    hasGPS = ReadTime(RMC + Index[0]) > 0;
    if (ReadDate(RMC + Index[8]) < 0)
      setDefaultDate();
  ...
  */
  ...
  {                                                                 // ClimbRate=0; hasClimb=0;
    ...
    if (RefPos.FixQuality == 0)
      return 0;                              // give up if no fix on the reference position
    int16_t TimeDiff = calcTimeDiff(RefPos); // [ms] time difference between positions
    if (TimeDiff < 10)
      return 0;                          // [ms] give up if smaller than 10ms (as well when negative)
    TurnRate = Heading - RefPos.Heading; // [0.1deg/s] turn rate
    if (TurnRate > 1800)
      TurnRate -= 3600;
    else if (TurnRate < (-1800))
      TurnRate += 3600;                                                               // wrap-around
    ...
    }                                 // take average of the GPS and baro climb rate
  ...
  void Write(mavlink_gps_raw_int_t *MAV) const
  {
  ...
  void Read(const mavlink_gps_raw_int_t *MAV, uint64_t UnixTime_ms = 0)
  {
    if (UnixTime_ms)
    {
      setUnixTime_ms(UnixTime_ms);
      hasTime = 1;
    }
  ...
  void Read(const mavlink_global_position_int_t *MAV, uint64_t UnixTime_ms = 0)
  {
    if (UnixTime_ms)
    {
      setUnixTime_ms(UnixTime_ms);
  ...
  void Read(const mavlink_scaled_pressure_t *MAV, uint64_t UnixTime_ms = 0)
  {
    if (UnixTime_ms)
    {
      setUnixTime_ms(UnixTime_ms);
  ...
  void EncodeAirPos(FANET_Packet &Packet, uint8_t AcftType = 1, bool Track = 1)
  {
    ...
    if (hasBaro)
    {
      Packet.setQNE((StdAltitude + 5) / 10);
    }
  }
  ...
  void Encode(OGNx_Packet &Packet) const
  {
    ...
    if ((FixQuality > 0) && (FixMode >= 2))
      Packet.Position.FixMode = FixMode - 2; //
    else
      Packet.Position.FixMode = 0;
    if (PDOP > 0)
    ...
    if (mSec >= 500)
    {
      ...
      if (ShortTime >= 60)
        ShortTime -= 60;
    }                                           // round to the closest full second
    ...
    if (hasBaro)
      Packet.EncodeStdAltitude((StdAltitude + 5) / 10); // Pressure altitude
    else
      Packet.clrBaro(); // or no-baro if pressure sensor data not there
  }
  ...
  */
  ...
  void Encode(OGNx_Packet &Packet, int16_t dTime) const // Encode position which is extrapolated by the given fraction of a second
  {
    ...
    if (hasBaro)
      Packet.EncodeStdAltitude((StdAltitude + (Alt - Altitude) + 5) / 10); // Pressure altitude
    else
      Packet.clrBaro(); // or no-baro if pressure sensor data not there
  }
  ...
  int32_t calcLatitudeExtrapolation(int32_t Time, int32_t LatSpeed) const // [ms] [0.1m/s]
  {
    return (Time * LatSpeed * 283 + 0x40000) >> 19;
  } // [0.1m]
  ...
  static int16_t calcLatCosine(int16_t LatAngle)
  {
    ...
    if (LatCos <= 0)
      LatCos = 1; // protect against zero as it is used for division
    return LatCos;
  }
  ...
  int WriteAPRS(char *Out, const char *Call, const char *Icon, uint32_t ID)
  {
    ...
    Out[Len++] = 'A';
    Out[Len++] = '=';
    Len += Format_UnsDec(Out + Len, (MetersToFeet(Altitude) + 5) / 10, 6); // [feet] altitude
    Out[Len++] = ' ';
    Out[Len++] = '!';
    Out[Len++] = 'W';
    Out[Len++] = LatW;
    Out[Len++] = LonW;
    Out[Len++] = '!'; // more accurate Lat/Lon
    Out[Len++] = ' ';
    Out[Len++] = 'i';
    Out[Len++] = 'd';
    Len += Format_Hex(Out + Len, ID); // ID

    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, ((int32_t)ClimbRate * 10079 + 256) >> 9, 3);
    Out[Len++] = 'f';
    Out[Len++] = 'p';
    Out[Len++] = 'm'; // [fpm]
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, TurnRate / 3, 2, 1);
    Out[Len++] = 'r';
    Out[Len++] = 'o';
    Out[Len++] = 't'; // [ROT]
    ...
    if (hasBaro)
    {
      ...
      Out[Len++] = 'F';
      Out[Len++] = 'L';
      Len += Format_UnsDec(Out + Len, MetersToFeet((uint32_t)Alt), 5, 2);
    } // [feet] "Flight Level"

    uint16_t DOP = PDOP;
    if (DOP == 0)
      DOP = HDOP;
    uint16_t HorPrec = (DOP * 2 + 5) / 10;
    if (HorPrec > 63)
      HorPrec = 63; // [m]
    uint16_t VerPrec = (DOP * 3 + 5) / 10;
    if (VerPrec > 63)
      VerPrec = 63; // [m]
    Out[Len++] = ' ';
    Out[Len++] = 'g';
    Out[Len++] = 'p';
    Out[Len++] = 's';
    Len += Format_UnsDec(Out + Len, HorPrec);
    Out[Len++] = 'x';
    Len += Format_UnsDec(Out + Len, VerPrec);
  ...
  {                             // if(!isValid()) return 0;
    ...
    Out[Len++] = 'B';
    if (isTimeValid())
      Len += WriteHHMMSS(Out + Len); // if time is valid
    else
      Len += Format_String(Out + Len, "      "); // or leave empty
    if (isValid())                               // if position valid
    {
      ...
      Len += Format_String(Out + Len, "                    "); // is position not valid then leave empty
    if (hasBaro)                                               // if pressure data is there
    {
      ...
      if (Alt < 0)
      {
        Alt = (-Alt);
        Out[Len++] = '-';
        Len += Format_UnsDec(Out + Len, (uint32_t)Alt, 4);
      } // -AAAA (when negative)
      else
      {
        Len += Format_UnsDec(Out + Len, (uint32_t)Alt, 5);
      } // AAAAA
    }
    else
      Len += Format_String(Out + Len, "     ");
...
private:
  ...
  {
    int32_t Knots;
    if (Read_Float1(Knots, Value) < 1)
      return -1; // Speed: 0.1 knots
    Speed = (527 * Knots + 512) >> 10;
    return 0;
  } // convert speed to 0.1 meter/sec

  int8_t ReadHeading(const char *Value)
  {
    return Read_Float1(Heading, Value);
  } // Heading units: 0.1 degree

  int8_t ReadPDOP(const char *Value)
  {
    int16_t DOP;
    if (Read_Float1(DOP, Value) < 1)
      return -1;
    if (DOP < 10)
      DOP = 10;
    else if (DOP > 255)
...
public:
  int8_t static IndexNMEA(uint8_t Index[20], const char *Seq) // index parameters and verify the NMEA checksum
  {
    ...
    uint8_t Check = 0;
    if (Seq[Ptr] != '$')
      return -1; // first chat. must be dollar sign
    Ptr++;
    for (; Ptr <= 6; Ptr++) // go through the sentence name
    {
      if (Seq[Ptr] == ',')
        break; // stop at comma
      Check ^= Seq[Ptr];
    } // take char. to checksum
    if (Seq[Ptr] != ',')
      return -1;         // comma after the sentence name
    Check ^= Seq[Ptr++]; // take comma to the checksum
    Index[0] = Ptr;
    int8_t Params = 1; // first parameter
    for (;;)
    {
      char ch = Seq[Ptr++];
      if (ch < ' ')
        return -1; // go through the chars
      if (ch == '*')
        break;     // break at star (check-sum should follow)
      Check ^= ch; // get chars to the checksum
      if (ch == ',')
      {
        Index[Params++] = Ptr;
      } // if comma then counr next parameter
    }
    if (Seq[Ptr++] != HexDigit(Check >> 4))
      return -2; // verify checksum
    if (Seq[Ptr++] != HexDigit(Check & 0x0F))
      return -2;
    // printf("%s => [%d]\n", Seq, Params);
    return Params;
  }
