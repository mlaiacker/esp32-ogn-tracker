...
public:
  ...
  uint16_t getAirTime(uint8_t PktLen) // [ms] estimated time on the air for given packet length
  {
    ...
    uint8_t NibPerBlock = SF;
    if (LowRate)
      NibPerBlock -= 2;                                         // [nibbles/block] byte-halfs
...
public:
  ...
  { // uint8_t ManchErr = Count1s(RxPktErr, 26);
    ...
    CONS_UART_Write(' ');
    Format_Hex(CONS_UART_Write, Channel);
    CONS_UART_Write('/');
    ...
    {
      CONS_UART_Write(' ');
      Format_Hex(CONS_UART_Write, Err[Idx]);
    }
    CONS_UART_Write('\r');
    CONS_UART_Write('\n');
  }
...
public:              // hardware access functions
  ...
  {
    Block_Buffer[0] = Addr | 0x80;
    memcpy(Block_Buffer + 1, Data, Len);
  ...
  uint16_t WaitWhileBusy(uint16_t Loops = 100) // 50 seems to be still too short on RPI
  {
    ...
    return Loops;
  } // return number of looks left or zero (=> still busy)
  ...
  uint8_t *Regs_Write(uint16_t Addr, const uint8_t *Data, uint8_t Len) // register-write code, 2-byte Address, Data[Len]
  {
    ...
    Format_String(CONS_UART_Write, ");\n");
#endif
    Block_Buffer[0] = CMD_WRITEREGISTER;
    Block_Buffer[1] = Addr >> 8;
    Block_Buffer[2] = Addr;
    memcpy(Block_Buffer + 3, Data, Len);
  ...
  {
    WaitWhileBusy_ms(); // if(WaitWhileBusy()==0) return 0;
    Block_Buffer[0] = CMD_READREGISTER;
    Block_Buffer[1] = Addr >> 8;
    Block_Buffer[2] = Addr;
    memset(Block_Buffer + 3, 0, Len + 1);
  ...
  uint8_t *Buff_Write(uint8_t Ofs, const uint8_t *Data, uint8_t Len) // buffer-write code, 1-byte offset, Data[Len]
  {
    ...
    Format_String(CONS_UART_Write, ");\n");
#endif
    Block_Buffer[0] = CMD_WRITEBUFFER;
    Block_Buffer[1] = Ofs;
    memcpy(Block_Buffer + 2, Data, Len);
    (*TransferBlock)(Block_Buffer, Len + 2);
    return Block_Buffer + 2;
  }
  ...
  {
    WaitWhileBusy_ms(); // if(WaitWhileBusy()==0) return 0;
    Block_Buffer[0] = CMD_READBUFFER;
    Block_Buffer[1] = Ofs;
    memset(Block_Buffer + 2, 0, Len + 1);
    (*TransferBlock)(Block_Buffer, Len + 3);
    return Block_Buffer + 3;
  }
#endif
  ...
  {
    return (*Busy_isOn)();
  } // read the BUSY line of the sx1262
#endif
...
public:
  ...
  void setFrequency(uint32_t Freq) // [Hz] set for given frequency
  {
    ...
    Freq += Corr;
    WriteFreq(Freq);
  } // [32MHz/2^27] write into the RF chip
  ...
  static uint16_t SwapBytes(uint16_t Word)
  {
  ...
  {                                                 // printf("WriteByte(0x%02X => [0x%02X])\n", Byte, Addr);
    uint8_t *Ret = Block_Write(&Byte, 1, Addr);
    return *Ret;
  }

  void WriteWord(uint16_t Word, uint8_t Addr = 0) // write Word => two bytes
  {                                               // printf("WriteWord(0x%04X => [0x%02X])\n", Word, Addr);
    uint16_t Swapped = SwapBytes(Word);
    Block_Write((uint8_t *)&Swapped, 2, Addr);
  }
  ...
  void OGN_ReadPacket(uint8_t *Data, uint8_t *Err, uint8_t Len = 26) // read packet data from FIFO
  {
    ...
    for (uint8_t Idx = 0; Idx < Len; Idx++) // loop over packet bytes
    {
      ...
      uint8_t ErrH = ByteH >> 4;
      ByteH &= 0x0F; // decode manchester, detect (some) errors
      uint8_t ByteL = Packet[PktIdx++];
      ByteL = ManchesterDecode[ByteL];
      uint8_t ErrL = ByteL >> 4;
      ByteL &= 0x0F;
...
public:
  uint32_t WriteFreq(uint32_t Freq) const // [32MHz/2^19] Set center frequency in units of RFM69 synth.
  {
    ...
    Deselect();
    return Old << 8;
  } // return the previously set frequency
  ...
  void ReadPacketOGN(uint8_t *Data, uint8_t *Err, uint8_t Len = 26) const // read packet data from FIFO
  {
    ...
    Deselect();
  } // de-select RF chip: end of SPI transfer
  ...
  {
    if (SyncTol > 7)
      SyncTol = 7; // no more than 7 bit errors can be tolerated on SYNC
    if (WriteSize > 8)
      WriteSize = 8;                                                    // up to 8 bytes of SYNC can be programmed
    ...
    WriteWord(/* 9-WriteSize, */ 1, REG_PREAMBLEMSB);
...
//              ^ 8 or 9 ?
  ...
  void WriteTxPower(int8_t TxPower, uint8_t TxRamp = 0x04) // 0x04 = 200us ramp time
  {
    ...
    Cmd_Write(CMD_SETTXPARAMS, TxParm, 2);
  } // 0x8E, Power, RampTime
  ...
  static void Pack3bytes(uint8_t *Byte, uint32_t Value)
  {
    Byte[0] = Value >> 16;
    Byte[1] = Value >> 8;
    Byte[2] = Value;
  ...
  {
    WriteTxPower(0);
    RFM_LoRa_Config CFG = RFM_FNTcfg;
    CFG.CR = CR;
  ...
  void FSK_WriteSYNC(uint8_t WriteSize, uint8_t SyncTol, const uint8_t *SyncData)
  {
    ...
    Regs_Write(REG_SYNCWORD0, SyncData + (8 - WriteSize), WriteSize);
  } // Write the SYNC word
  ...
  void OGN_Configure(int16_t Channel, const uint8_t *SyncData)
  {
    ...
    Cmd_Write(CMD_SETPACKETPARAMS, Param, 9);     // 0x8C, PacketParam
    setDioMode(/* IRQ_TXDONE | */ IRQ_RXDONE);
    Regs_Write(REG_SYNCWORD0, SyncData, 8);
  } // Write the SYNC word
  ...
  void PAW_Configure(const uint8_t *Sync)
  {
    ...
    Cmd_Write(CMD_SETPACKETPARAMS, Param, 9);     // 0x8C, PacketParam
    setDioMode(/* IRQ_TXDONE | */ IRQ_RXDONE);
    Regs_Write(REG_SYNCWORD0, Sync, 8);
  } // Write the SYNC word

  void ClearIrqFlags(uint16_t Mask = IRQ_ALL)
  {
    uint8_t Data[2];
    Data[0] = Mask >> 8;
    Data[1] = Mask;
    Cmd_Write(CMD_CLEARIRQSTATUS, Data, 2);
  }
  uint16_t ReadIrqFlags(void)
  {
    uint8_t *Stat = Cmd_Read(CMD_GETIRQSTATUS, 2);
    return (((uint16_t)(Stat[0])) << 8) | Stat[1];
  }
  ...
  {
    uint8_t T[3];
    Pack3bytes(T, Timeout);
    Cmd_Write(CMD_SETTX, T, 3);
  }
  void setModeRX(uint32_t Timeout = 0)
  {
    uint8_t T[3];
    Pack3bytes(T, Timeout);
    Cmd_Write(CMD_SETRX, T, 3);
  }
  ...
  {
    Format_String(CONS_UART_Write, "SX1262 status:");
    uint8_t RxMode = getStatus();
  ...
  void WriteTxPower(int8_t TxPower, bool isHW)
  {
    ...
    if (isHW)
      WriteTxPower_HW(TxPower);
    else
      WriteTxPower_W(TxPower);
  }
  ...
  void OGN_Configure(int16_t Channel, const uint8_t *Sync)
  {
    ...
    WriteByte(+10, REG_TESTAFC);
  } // [0x00] [488Hz] if AfcLowBetaOn
#endif
...
// #ifdef WITH_RFM95
  ...
  { // WriteMode(RF_OPMODE_LORA_STANDBY);
    ...
    for (Check = 0; Check < Wait; Check++)
    {
      ...
      if (Mode != RF_OPMODE_LORA_TX)
        break;
    }
    return Check + 1;
  } // afterwards just wait for TX mode to stop
  ...
  int LoRa_ReceivePacket(RxPacket &Packet)
  {
    ...
    if (FreqOfs & 0x00080000)
      FreqOfs |= 0xFFF00000; // extend the sign bit
    else
      FreqOfs &= 0x000FFFFF;
  ...
  {
    chipVer = ReadByte(REG_VERSION);
    return chipVer;
  }                                                          // 0x24 for RFM69 or 0x12 for RFM95
  ...
  {
    uint8_t *RSSI = Cmd_Read(CMD_GETRSSIINST, 1);
    return RSSI[0];
  }
#endif

#ifdef WITH_RFM69
  void TriggerRSSI(void)
  {
    WriteByte(0x01, REG_RSSICONFIG);
  }                                                                   // trigger measurement
  ...
  {
    chipTemp = 165 - ReadByte(REG_TEMP2);
    return chipTemp;
  } // [deg]
#endif
