...
public:
  ...
  uint32_t getTime(uint32_t RefTime, int FwdMargin = 5) const
  {
    ...
    if (DiffSec > FwdMargin)
      DiffSec -= 60; // difference should always be zero or negative, but can be small positive for predicted positions
    else if (DiffSec <= (-60 + FwdMargin))
      DiffSec += 60;
    return RefTime + DiffSec;
  } // get out the correct position time
  ...
  uint8_t Read(const char *Inp)
  {
    ...
    if (Inp[0] == ' ')
      Inp++;
    int Chars = Read_Hex(HeaderWord, Inp);
    if (Chars != 8)
      return 0;
    Inp += Chars;
    Len += 4;
    for (uint8_t Idx = 0; Idx < 4; Idx++)
    {
      if (Inp[0] == ' ')
        Inp++;
      int Chars = Read_Hex(Data[Idx], Inp);
      if (Chars != 8)
        return 0;
      Inp += Chars;
      Len += 4;
  ...
  uint8_t Dump(char *Out)
  {
    ...
    {
      Out[Len++] = ' ';
      Len += Format_Hex(Out + Len, Data[Idx]);
    }
    return Len;
  }
  ...
  uint8_t WriteStatus(char *Out)
  {
    ...
    Out[Len++] = ' ';
    Out[Len++] = 'h';
    Len += Format_Hex(Out + Len, (uint8_t)Status.Hardware);
    Out[Len++] = ' ';
    Out[Len++] = 'v';
    Len += Format_Hex(Out + Len, (uint8_t)Status.Firmware);
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, Status.Satellites);
    Out[Len++] = 's';
    Out[Len++] = 'a';
    Out[Len++] = 't';
    Out[Len++] = '/';
    Out[Len++] = '0' + Status.FixQuality;
    Out[Len++] = '/';
    Len += Format_UnsDec(Out + Len, Status.SatSNR + 8);
    Out[Len++] = 'd';
    Out[Len++] = 'B';
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, DecodeAltitude(), 1, 0, 1);
    Out[Len++] = 'm';
    if (Status.Pressure > 0)
    {
      Out[Len++] = ' ';
      Len += Format_UnsDec(Out + Len, (((uint32_t)Status.Pressure << 3) + 5) / 10, 2, 1);
      Out[Len++] = 'h';
      Out[Len++] = 'P';
      Out[Len++] = 'a';
    }
    ...
    {
      Out[Len++] = ' ';
      Len += Format_SignDec(Out + Len, DecodeTemperature(), 2, 1);
      Out[Len++] = 'd';
      Out[Len++] = 'e';
      Out[Len++] = 'g';
      Out[Len++] = 'C';
    ...
    {
      Out[Len++] = ' ';
      Len += Format_SignDec(Out + Len, DecodeHumidity(), 2, 1);
      Out[Len++] = '%';
    }
    Out[Len++] = ' ';
    Len += Format_SignDec(Out + Len, ((uint32_t)DecodeVoltage() * 100 + 32) >> 6, 3, 2);
    Out[Len++] = 'V';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, Status.TxPower + 4);
    Out[Len++] = '/';
    Out[Len++] = '-';
    Len += Format_UnsDec(Out + Len, 5 * Status.RadioNoise, 2, 1);
    Out[Len++] = 'd';
    Out[Len++] = 'B';
    Out[Len++] = 'm';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, (1 << Status.RxRate) - 1);
    Out[Len++] = '/';
    Out[Len++] = 'm';
    Out[Len++] = 'i';
    Out[Len++] = 'n';
  ...
  */
  ...
  /*
     ...
     { MAV->ICAO_address = HeaderWord&0x03FFFFFF;
       ...
       MAV->emiter_type   =    0; }
  */
  void Encode(mavlink_adsb_vehicle_t *MAV)
  {
    ...
    MAV->emitter_type = EmitterType[Position.AcftType];                          // convert from the OGN
  }
  ...
  int ReadAPRS(const char *Msg) // read an APRS position message
  {
    ...
    if (Data == 0)
      return -1; // where the time/position data starts
    Data++;
    const char *Dest = strchr(Msg, '>');
    if (Dest == 0)
      return -1; // where the destination call is
    Dest++;
    ...
    uint8_t AddrType;
    uint32_t Address;
    if (memcmp(Msg, "RND", 3) == 0)
      AddrType = 0;
    else if (memcmp(Msg, "ICA", 3) == 0)
      AddrType = 1;
    else if (memcmp(Msg, "FLR", 3) == 0)
      AddrType = 2;
    else if (memcmp(Msg, "OGN", 3) == 0)
      AddrType = 3;
    else
      AddrType = 4;
    if (AddrType < 4)
    {
      if (Read_Hex(Address, Msg + 3) == 6)
    ...
    {
      Sec = Read_Dec2(Data + 5);
      if (Sec < 0)
        return -1;
      Min = Read_Dec2(Data + 3);
      if (Min < 0)
        return -1;
      Hour = Read_Dec2(Data + 1);
      if (Hour < 0)
        return -1;
    }
    ...
    {
      Sec = 0;
      Min = Read_Dec2(Data + 5);
      if (Min < 0)
        return -1;
      Hour = Read_Dec2(Data + 3);
      if (Hour < 0)
        return -1;
    }
    else
    ...
    int8_t LatDeg = Read_Dec2(Data);
    if (LatDeg < 0)
      return -1;
    int8_t LatMin = Read_Dec2(Data + 2);
    if (LatMin < 0)
      return -1;
    if (Data[4] != '.')
      return -1;
    int8_t LatFrac = Read_Dec2(Data + 5);
    if (LatFrac < 0)
      return -1;
    ...
    int16_t LonDeg = Read_Dec3(Data);
    if (LonDeg < 0)
      return -1;
    int8_t LonMin = Read_Dec2(Data + 3);
    if (LonMin < 0)
      return -1;
    if (Data[5] != '.')
      return -1;
    int8_t LonFrac = Read_Dec2(Data + 6);
    if (LonFrac < 0)
      return -1;
    ...
    for (;;)
    {
      ...
      {
        int32_t Value;
        int8_t Len = Read_Float1(Value, Data);
        if (Len > 0)
        {
          Data += Len;
          if (memcmp(Data, "fpm", 3) == 0)
          {
            EncodeClimbRate((333 * Value + 0x8000) >> 16);
            Data += 3;
            continue;
          }
          if (memcmp(Data, "rot", 3) == 0)
          {
            EncodeTurnRate(3 * Value);
            Data += 3;
            continue;
          }
        }
      ...
      if ((Data[0] == 'g') && (Data[1] == 'p') && (Data[2] == 's'))
      {
        ...
        if (HorPrec < 0)
          HorPrec = Read_Dec1(Data[3]);
        if (HorPrec >= 0)
        {
          uint16_t DOP = HorPrec * 5;
          if (DOP < 10)
            DOP = 10;
          else if (DOP > 230)
            DOP = 230;
  ...
  uint8_t WriteAPRS(char *Msg, uint32_t Time, const char *ProtName = "APRS") // write an APRS position message
  {
    ...
    memcpy(Msg + Len, AddrTypeName[Header.AddrType], 3);
    Len += 3;
    ...
    memcpy(Msg + Len, ProtName, ProtLen);
    Len += ProtLen;
    if (Header.Relay)
    {
      memcpy(Msg + Len, ",RELAY*", 7);
      Len += 7;
    }
    Msg[Len++] = ':';

    if (Header.NonPos && Status.ReportType > 1)
    {
      Msg[Len] = 0;
      return 0;
    } // give up if neither position nor status nor info
    ...
    {
      if (isStatus())
        Len += WriteStatus(Msg + Len);
      else if (isInfo())
        Len += WriteDeviceInfo(Msg + Len);
      /* Msg[Len++]='\n'; */ Msg[Len] = 0;
      return Len;
    }
    ...
    Msg[Len++] = 'A';
    Msg[Len++] = '=';
    Len += Format_UnsDec(Msg + Len, MetersToFeet(DecodeAltitude()), 6);
    ...
    {
      Msg[Len++] = ' ';
      Len += Format_SignDec(Msg + Len, ((int32_t)DecodeClimbRate() * 10079 + 256) >> 9, 3);
      Msg[Len++] = 'f';
      Msg[Len++] = 'p';
      Msg[Len++] = 'm';
    }
    ...
    {
      Msg[Len++] = ' ';
      Len += Format_SignDec(Msg + Len, DecodeTurnRate() / 3, 2, 1);
      Msg[Len++] = 'r';
      Msg[Len++] = 'o';
      Msg[Len++] = 't';
    }
    ...
    Msg[Len++] = 'p';
    Msg[Len++] = 's';
    Len += Format_UnsDec(Msg + Len, HorPrec);
    Msg[Len++] = 'x';
    Len += Format_UnsDec(Msg + Len, VerPrec);
  ...
  {
    uint8_t Len = 0;
    Out[Len++] = HexDigit(Position.AcftType);
    Out[Len++] = ':';
    Out[Len++] = '0' + Header.AddrType;
    Out[Len++] = ':';
    ...
    Out[Len++] = 'm';
    Out[Len++] = ' ';
    Len += Format_UnsDec(Out + Len, DecodeSpeed(), 2, 1);
    Out[Len++] = 'm';
    Out[Len++] = '/';
    Out[Len++] = 's';
    Out[Len++] = ' ';
  ...
  void EncodeSpeed(int16_t Speed) // speed in 0.2 knots (or 0.1m/s)
  {
    if (Speed < 0)
      ...
      Speed = UnsVRencode<uint16_t, 8>(Speed); // EncodeUR2V8(Speed);
  ...
  uint8_t getInfoChar(uint8_t Idx) const // get 7-bit Char from given position
  {
    ...
    uint8_t Ofs = BitIdx & 0x07;
    if (Ofs == 0)
      return Info.Data[Idx] & 0x7F;
    if (Ofs == 1)
      return Info.Data[Idx] >> 1;
    uint8_t Len = 8 - Ofs;
  ...
  uint8_t addInfo(const char *Value, uint8_t InfoType) // add an info field
  {
    ...
    if (Idx)
      Idx++; // if at least one already, then skip over the terminator
    if (Idx >= 15)
      return 0;
    uint8_t Len = 0;
    for (;;)
  ...
  uint8_t readInfo(char *Value, uint8_t &InfoType, uint8_t ValueIdx = 0) const
  {
    ...
    {
      if ((ValueIdx + Len) > Chars)
        return 0;                         // return failure if overrun the data
      Char = getInfoChar(ValueIdx + Len); // get the character
      if (Char < 0x20)
        break; // if less than 0x20 (space) then this is the terminator
      Value[Len++] = Char;
    }
  ...
  void Whiten(void)
  {
    TEA_Encrypt_Key0(Data, 8);
    TEA_Encrypt_Key0(Data + 2, 8);
  } // whiten the position
  void Dewhiten(void)
  {
    TEA_Decrypt_Key0(Data, 8);
