...
public:
  ...
  {
    if (isComplete())
      return;     // if already a complete frame, ignore extra bytes
    if (Len == 0) // if data is empty
    {
      if (Byte != '#')
        return;           // then ignore all bytes but '#'
  ...
  uint8_t isChecked(void) const // is the AERO checksum OK ?
  {
    if (Data[Len - 5] != ',')
      return 0;
    uint16_t CRC;
    if (Read_Hex(CRC, (const char *)(Data + (Len - 4))) != 4)
      return 0;
...
static void ADSB_AERO(bool GoodCRC)
{
  ...
  if (!GoodCRC)
    Format_String(CONS_UART_Write, " <- bad !");
  Format_String(CONS_UART_Write, "\n");
  xSemaphoreGive(CONS_Mutex);
  MAV_text("aero:%s (%i)", AERO.Data, AERO.Len);
  // #endif
}
    ...
    vTaskAERO(void *pvParameters)
{
  ...
  for (;;) // main task loop: every milisecond (RTOS time tick)
  {
    ...
    {
      uint8_t Byte;
      int Err = AERO_UART_Read(Byte);
      if (Err <= 0)
        break;                // get Byte from serial port, if no bytes then break this l$
