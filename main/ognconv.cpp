...
uint16_t DecodeUR2V8(uint16_t Value) // Decode 10bit 0..0x3FF
{
  ...
  Value &= 0x0FF;
  if (Range == 0)
    return Value; // 000..0FF
  if (Range == 1)
    return 0x101 + (Value << 1); // 100..2FE
  if (Range == 2)
    return 0x302 + (Value << 2); // 300..6FC
...
uint8_t DecodeUR2V4(uint8_t DOP)
{
  ...
  if (Range == 0)
    return DOP; // 00..0F
  if (Range == 1)
    return 0x11 + (DOP << 1); // 10..2E
  if (Range == 2)
    return 0x32 + (DOP << 2); // 30..6C
  return 0x74 + (DOP << 3);
...
uint16_t DecodeUR2V12(uint16_t Value)
{
  ...
  Value &= 0x0FFF;
  if (Range == 0)
    return Value; // 0000..0FFF
  if (Range == 1)
    return 0x1001 + (Value << 1); // 1000..2FFE
  if (Range == 2)
    return 0x3002 + (Value << 2); // 3000..6FFC
...
{
  int Len = 0;
  const char *Msg = strchr(Inp, ':');
  if (Msg == 0)
    return 0; // colon: separates header and message
  Msg++;      // where message starts
  if (Msg[0] != '/' || Msg[7] != 'h')
    return 0;
  const char *Pos = Msg + 8;
  if (Pos[4] != '.' || Pos[14] != '.')
    return 0; // where position starts
  const char *ExtPos = strstr(Pos + 18, " !W");
  if (ExtPos[5] == '!')
    ExtPos += 3;
  else
    ExtPos = 0;
  Out[Len++] = 'B'; // B-record
  memcpy(Out + Len, Msg + 1, 6);
  Len += 6; // copy UTC time
  memcpy(Out + Len, Pos, 4);
  Len += 4; // copy DDMM
  memcpy(Out + Len, Pos + 5, 2);
  Len += 2;                              // copy fractional MM
  Out[Len++] = ExtPos ? ExtPos[0] : '0'; // extended precision
  Out[Len++] = Pos[7];                   // copy N/S sign
  memcpy(Out + Len, Pos + 9, 5);
  Len += 5; // copy DDMM
  memcpy(Out + Len, Pos + 15, 2);
  Len += 2;                                 // copy fractional MM
  ...
  {
    int Alt = AltH * 100 + AltL;
    Alt = FeetToMeters(Alt);
    if (Alt < 0)
    {
      Alt = (-Alt);
      Out[Len] = '-';
      Format_UnsDec(Out + Len + 1, (uint32_t)Alt, 4);
    }
    else
    {
      Format_UnsDec(Out + Len, (uint32_t)Alt, 5);
    }
  }
  Len += 5;
  int32_t Alt = 0;                                                      //
  ...
  {
    Alt = FeetToMeters(Alt);
    Alt += GeoidSepar; // convert to meters and add GeoidSepar for HAE
    if (Alt < 0)
    {
      Alt = (-Alt);
      Out[Len] = '-';
  ...
  Out[Len] = 0;
  return Len;
} // add NL, terminator and return length og the string
