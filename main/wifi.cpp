...
bool WIFI_isConnected(void) {
	return WIFI_IP.ip.addr != 0;
}  // return "connected" status when IP from DHCP is there
bool WIFI_isAP(void) {
	return WIFI_Config.ap.ssid_len != 0;
}  // return if AP mode enabled
...
esp_err_t WIFI_Init(void) {
	...
	IP4_ADDR(&ipInfo.ip, WIFI_ADDRESS_IP1, WIFI_ADDRESS_IP2, WIFI_ADDRESS_IP3,
			WIFI_ADDRESS_IP4);	  	// Change IP in config.h
	IP4_ADDR(&ipInfo.gw, WIFI_ADDRESS_GW1, WIFI_ADDRESS_GW2, WIFI_ADDRESS_GW3,
			WIFI_ADDRESS_GW4);      	// Change GATEWAY in config.h
	IP4_ADDR(&ipInfo.netmask, WIFI_ADDRESS_MK1, WIFI_ADDRESS_MK2,
			WIFI_ADDRESS_MK3, WIFI_ADDRESS_MK4); 	// change MASK in config.h
...
esp_err_t WIFI_StartAP(const char *SSID, const char *Pass, int MaxConnections) {
	...
	WIFI_Config.ap.authmode =
			(Pass && Pass[0]) ? WIFI_AUTH_WPA_WPA2_PSK : WIFI_AUTH_OPEN;
	...
	if (Err != ESP_OK)
		return Err;     // which one should come first: set_mode or set_config ?
...
esp_err_t WIFI_ActiveScan(wifi_ap_record_t *AP, uint16_t &APs) {
	...
	wifi_scan_config_t Config = { ssid:0, bssid:0, channel:0, show_hidden:0,
			scan_type:WIFI_SCAN_TYPE_ACTIVE, scan_time: { active: { min:100, max
					:2000 } } };
	Err = esp_wifi_scan_start(&Config, 1);
	if (Err != ESP_OK)
		return Err;
	Err = esp_wifi_scan_get_ap_records(&APs, AP);
	return Err;
}
...
esp_err_t WIFI_PassiveScan(wifi_ap_record_t *AP, uint16_t &APs) //
	...
	esp_err_t Err;
	wifi_scan_config_t Config = { ssid:0, bssid:0, channel:0, show_hidden:0,
			scan_type:WIFI_SCAN_TYPE_PASSIVE, scan_time: { passive: 100 } };
	Err = esp_wifi_scan_start(&Config, 1);
	if (Err != ESP_OK)
		return Err;
	Err = esp_wifi_scan_get_ap_records(&APs, AP);
...
uint32_t WIFI_getLocalIP(void)          // get local IP, once DHCP phase is done
		{
	esp_err_t Err;
	Err = tcpip_adapter_get_ip_info(TCPIP_ADAPTER_IF_STA, &WIFI_IP);
	if (Err != ESP_OK)
		return 0;
...
uint8_t AP_Print(char *Out, wifi_ap_record_t *AP) // print numbers for a found Wi-Fi Access Point
	...
	for (uint8_t Idx = 0; Idx < 6; Idx++) {
		Len += Format_Hex(Out + Len, AP->bssid[Idx]);
		Out[Len++] = ':';
	}
