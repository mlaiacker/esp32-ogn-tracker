...
void PrintTasks(void (*CONS_UART_Write)(char))
{
  ...
  for (UBaseType_t T = 0; T < uxArraySize; T++)
  {
    ...
    //   Line[Len++]=' ';
    Len += Format_UnsDec(Line + Len, Task->uxCurrentPriority, 2);
    Line[Len++] = ' ';
...
{
  if (VerboseSuspendTime == 0)
    return;
  uint32_t Time = TimeSync_Time() - VerboseSuspendTime;
  if (Time > VerboseSuspendTimeout)
  {
    Parameters.Verbose = Verbose;
    VerboseSuspendTime = 0;
  }
}

static void ProcessCtrlV(void)
{
  if (VerboseSuspendTime)
  {
    Parameters.Verbose = Verbose;
    VerboseSuspendTime = 0;
    return;
  ...
  VerboseSuspendTime = TimeSync_Time();
...
static void ProcessCtrlF(void) // list log files to the console
{
  ...
  if (Dir)
  {
    for (;;) // loop over files
    {
      ...
      if (!Ent)
        break; // get the next file of the directory
      if (Ent->d_type != DT_REG)
        continue; // if not a regular file (directory, link, ...): then skip
      char *Name = Ent->d_name;
      strcpy(FullName + 8, Name);
      if (stat(FullName, &Stat) < 0)
        continue;
...
static void ProcessCtrlC(void) // print system state to the console
{
  ...
  if (GPS_Status.NMEA)
    Format_String(CONS_UART_Write, ",NMEA");
  if (GPS_Status.UBX)
    Format_String(CONS_UART_Write, ",UBX");
  if (GPS_Status.MAV)
    Format_String(CONS_UART_Write, ",MAV");
  if (GPS_Status.BaudConfig)
    Format_String(CONS_UART_Write, ",BaudOK");
  if (GPS_Status.ModeConfig)
    Format_String(CONS_UART_Write, ",ModeOK");
  CONS_UART_Write('\r');
  CONS_UART_Write('\n');
...
static void ProcessInput(void)
{
  for (;;)
  {
    ...
    if (Byte == 0x06)
      ProcessCtrlF(); // if Ctrl-F received: list files
    if (Byte == 0x0B)
      ProcessCtrlK(); // if Ctrl-K received: print public key
    if (Byte == 0x0C)
    ...
    if (NMEA.isComplete())  // if complete NMEA:
    {
      ...
      NMEA.Clear();
    } // clear the NMEA processor for the next sentence
#ifdef WITH_GPS_UBX_PASS
...
extern "C" void vTaskCTRL(void *pvParameters)
{
    ...
    */
    ...
    if (TimeChange)
    {
      ...
      for (uint8_t Reg = 0; Reg <= 10; Reg++)
      {
        ...
        CONS_UART_Write(' ');
        Format_Hex(CONS_UART_Write, Val);
      }
